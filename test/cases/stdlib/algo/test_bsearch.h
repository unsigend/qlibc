...
int
person_compare_id(const void *a, const void *b)
{
...
int
person_compare_age(const void *a, const void *b)
{
  const struct person *pa = (const struct person *)a;
  const struct person *pb = (const struct person *)b;
  if (pa->age < pb->age)
...
UTEST_TEST_CASE(bsearch)
{
  ...
  {
    struct person people[] = { { 1, "Alice", 25 },
                               { 2, "Bob", 30 },
                               { 3, "Charlie", 35 },
                               { 4, "David", 40 },
                               { 5, "Eve", 45 } };
    ...
    void *result
        = bsearch(&key, people, 5, sizeof(struct person), person_compare_id);
    EXPECT_NOT_NULL(result);
  ...
  {
    struct person sorted[] = { { 1, "Charlie", 35 },
                               { 2, "Eve", 45 },
                               { 3, "Bob", 30 },
                               { 4, "David", 40 },
                               { 5, "Alice", 25 } };
    struct person key = { 3, "", 0 };
    void *result
        = bsearch(&key, sorted, 5, sizeof(struct person), person_compare_id);
    EXPECT_NOT_NULL(result);
    struct person *found = (struct person *)result;
    EXPECT_TRUE(found->id == 3);
  }

  {
    ...
    void *result
        = bsearch(&key, people, 5, sizeof(struct person), person_compare_age);
    EXPECT_NOT_NULL(result);
    struct person *found = (struct person *)result;
