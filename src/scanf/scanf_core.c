...
static int
instr(const char *str, char *dest, size_t width, struct fsm_t *fsm, int bhvs)
{
  const char *p = str;
  size_t total = 0;
  size_t n = 0;
...
static int
innum(const char *restrict str, struct fsm_t *fsm, void *dest, int base,
...
/* Internal core implementation of formatted input. With parser and
   formatter support. */
int
scanf_core(const char *restrict buff, const char *restrict fmt, va_list vlist,
           const char **end)
{
            ...
            case SPEC_C:
              {
                ...
                POP((void **)&c, &ap);
                int n
                    = instr(ctx.cursor, c, fsm.width ? fsm.width : 1, &fsm, 0);
                if (n == -1)
                  {
                    fail = 1;
                    goto done;
